...
class RearTruthDareWheelActivity : ComponentActivity() {

    private companion object {
        const val TAG = "RearTruthDareWheel"
    ...
    private fun WheelBody() {
        ...
        val minSafeStartDp = with(density) { TRUTH_DARE_SAFE_LEFT_PX.toDp() }
        val safeStartDp = minSafeStartDp.coerceAtLeast(cutoutLeftDp)
        val safeStartPx = with(density) { safeStartDp.toPx() }
        TruthDareRearGameScreen(safeStartPx = safeStartPx)
    }
    ...
    override fun onStart() {
        super.onStart()
        projectionSetup.onStart(this, RearTruthDareWheelActivity::class.java)
    }

    override fun onDestroy() {
        cancelRearUiInitPoll()
        projectionSetup.onDestroy(this, RearTruthDareWheelActivity::class.java)
        super.onDestroy()
    }

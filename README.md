...
<h1 align="center">MiRoot V2.4</h1>
...
- **手势注入正则修复**：修正 `removeGestureFromVarConfig` 中 Kotlin 字符串模板插值错误导致的 `OnOff` name 属性正则匹配失败
- **手势注入幂等性**：`patchManifestForGesture` 与 `patchVarConfigForGesture` 新增清理后检查，避免 `runCatching` 静默失败时重复注入

### V2.2 亮点

- **背屏平衡小游戏**：倾斜手机让小球留在圆圈内，含简单/普通/困难与水平仪模式；支持手势一键投屏至背屏
- **背屏真心话大冒险**：转盘选人、轮询或随机指定玩家；真心话/大冒险可转盘或直出；支持 TXT 批量导入自定义题库与题库管理
- **背屏蓝牙心率广播**：连接 BLE 心率带后在背屏实时显示 BPM，可配合手势/投屏链路启动
- **车控油耗与历史优化**：星瑞油耗解析与历史展示改进；油价区域偏好与油箱分析优化
- **背屏歌词分词修复**：修复分词显示模式下歌词反复闪烁、布局来回切换的问题
- **背屏投屏与手势优化**：官方手势策略、Compose 投屏初始化与多模块启动链路整合优化
- **主题注入增强**：更细粒度的注入失败原因提示，manifest 兼容性校验改进
- **配置备份增强**：云备份同步真心话自定义题库与相关偏好
...
推荐文件名：`MiRoot_2.4_Release.apk`（与版本号对应）
...
| 文档 | 说明 |
|------|------|
| [V2.4 发布说明](docs/RELEASE_v2.4.md) | 最新版本更新详情 |

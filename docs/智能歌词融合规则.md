...
| 网易云 | **网易 API** → 酷狗 → qsgc → lrclib ∥ lyrics.ovh |
| QQ 音乐 | **QQ API** → 酷狗 → qsgc → lrclib ∥ lyrics.ovh |
| 酷我手机版 / 车机网络兜底 | **酷我 API** → 酷狗 → qsgc → lrclib ∥ lyrics.ovh |
| 酷狗及其他 | **酷狗** → qsgc → lrclib ∥ lyrics.ovh |

**酷我车机版** `cn.kuwo.kwmusiccar`：智能切换下仍 **优先广播 / AUDIO_LYRIC**，上表网络链仅在专用通道失败后的兜底阶段生效。
...
**`computeFusedWordHighlightTarget` 分母陷阱（V3.17+ 已修复）**

```java
// ❌ 旧代码：totalWeight 在循环中累加，break 时后续未唱字未被计入分母
for (int i = 0; i < words.size(); i++) {
    totalWeight += weight;
    if (positionMs >= end) { ... }
    else if (positionMs > start) {
        sungWeight += weight * inWord;
        break;           // break 后 totalWeight 不再增加
    } else { break; }
}
return sungWeight / totalWeight;  // 分母过小 → 进度虚高

// ✅ 新代码：先遍历全部字计算总权重，再算已唱权重
totalWeight = preComputeAllWordsWeight();
for (int i = 0; i < words.size(); i++) {
    // ... sungWeight 累加, break 不影响 totalWeight
}
return sungWeight / totalWeight;  // 分母包含全部字 → 进度正确
```

**例**：10 字每字权重 1.0，实际只唱到第 1 个字 80% 处。旧代码 `totalWeight=1.0` → 进度 80%（虚高，显示为几乎唱完）；新代码 `totalWeight=10.0` → 进度 8%（正确，仅首字部分高亮）。

编辑 `computeFusedWordHighlightTarget` 时注意保持两段遍历的权重逻辑一致。

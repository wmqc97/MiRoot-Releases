...
1. `beginLyricsAcquisitionForTrack`：异步拉网络 API，并 `startSuperLyricRealtimeTicker` 监听推送。
2. 网络命中 → `unlockFromSuperLyricFallbackAndApplyNetworkLyrics`：写入完整 `enhancedLyricLines`，`currentLyricsSource = NETWORK_API`，关闭单行兜底模式。
...
- 有 `payload.hasValidWords()`：优先 `MixedLyricsLineMatcher` 行级融合；未通过则挂到 **播放进度锚定的当前行**（不改行文本）。
...
| 场景 | 行为 |
|------|------|
| 跨曲 SuperLyric 回调 | `isSuperLyricRealtimeTrackMatched` 拦截 |
